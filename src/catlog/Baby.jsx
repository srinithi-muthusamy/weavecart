...
const table = () => {
  ...
  useEffect(() => {
    const fetchtableProducts = async () => {
      try {
        const cachedProducts = sessionStorage.getItem("tableProducts");
        if (cachedProducts) {
        ...
        settableProducts(products);
        setFilteredProducts(products);
        sessionStorage.setItem("tableProducts", JSON.stringify(products));
      } catch (err) {
  ...
  const handleAddToCart = async (product) => {
    ...
    if (!token) {
      alert('Please login or sign up to add items to the cart.');
      navigate("/account");
      return;
    }
    ...
    try {
      ...
      // Update local storage instead of calling a non-existent endpoint
  ...
  return (
    <Container className="mt-5 pt-5">
      <div className="d-flex justify-content-between align-items-center mb-4">
        <h2 className="text-primary">table Products</h2>
